...
{ find minimum difference between angles }
function angdif(a, b: real): real; external; 
{ find vector distance }
function dist(x1, y1, x2, y2: integer): integer; external;
{ draw real box }
procedure boxr(x1, y1, x2, y2: integer; c: color; r: region); external;
{ draw real bold box }
procedure bboxr(x1, y1, x2, y2: integer; c: color; r: region); external;
{ draw real line }
procedure liner(x1, y1, x2, y2: integer; c: color; r: region); external; 
{ draw real bold line }
procedure bliner(x1, y1, x2, y2: integer; c: color; r: region); external; 
{ draw pie real }
procedure pier(xc, yc, r: integer; c: color; cr: region); external;
{ draw arc real }
procedure arcr(xs, ys, xe, ye, xc, yc, r: integer; c: color; 
               cr: region); external;
procedure setpix(vp: viewport; x, y: integer; c: color); external; { set pixel value }
{ place vector character }
procedure vchar(x, y: integer; c: char; s: integer; cl: color; 
                rt: boolean; r: region); external;
...
{ delete wire from all lists }
procedure delwire(w: drwptr); external;
...
procedure lnkjun(j: drwptr); external; { link junction }
procedure butact(b: buttyp); external; { set button active }
procedure butina(b: buttyp); external; { set button inactive }
{ clip line to window }
procedure clipb(var x1, y1, x2, y2: integer; var inside: boolean;
               mix, miy, mxx, mxy: integer); external;
...
{ draw connector }
procedure conr(x, y, r: integer; c: color; cr: region); external; 
...
procedure resptr; external; { reset pointer device }
...
procedure intersect(ax1, ay1, ax2, ay2, 
          bx1, by1, bx2, by2: integer; 
          var rx1, ry1, rx2, ry2: integer; var rf: boolean); 
          external;
{ perform dot grid placement }
procedure dogrid(xl, yl: dotvec; c: color); external;
procedure do10sgrid(c: color); external; { draw 10's grid }
procedure dointer(p: drwptr); external; { create intersections }
{ draw trace }
procedure dtrace(ts: point; tl: trcptr; r: region); external;
{ draw analog trace }
procedure atrace(ts: point; tl: atrcptr; r: region); external;
{}
{**************************************************************
   ...
   with d^ do case typ of { figure }
      ...
      tline: if (l.s.x = l.e.x) or (l.s.y = l.e.y) then
                ...
                liner(rotx(x1, y1, x2, y2, ox, l.s.x, l.s.y, rt), 
                      ...
                      clr, curwin^.cs^.vp.v);
      { wire }
      twire: if (l.s.x = l.e.x) or (l.s.y = l.e.y) then
                { orthogonal line, use critical region }
...
begin
   ...
   if button[blines].act then { line grid active }
      do10sgrid(yellow); { place grid lines }
   { draw layers }
   ...
   if button[bdots].act then { dot grid active }
      dogrid(dgridsx, dgridsy, black); { place dots }
...
var r:                  region;  { screen coordinates }
    xi, yi, yis:        dotinx;  { index for grid vectors }
...
begin
   ...
   if button[blines].act then begin { line grid is active }
      ...
      xi := 1; { set 1st position }
      yi := 1;
      { find first grid position in region }
      while (lgridsx[xi] < r.s.x) and (lgridsx[xi] <> -1) do 
         xi := xi + 1; 
      while (lgridsy[yi] < r.s.y) and (lgridsy[yi] <> -1) do 
         yi := yi + 1; 
      { lay down grid }
      while (lgridsx[xi] <= r.e.x) and (lgridsx[xi] <> -1) do 
         begin { draw vertical lines }

         line(screen, lgridsx[xi], r.s.y, lgridsx[xi], r.e.y, yellow);
         xi := xi + 1 { next }

      end;
      while (lgridsy[yi] <= r.e.y) and (lgridsy[yi] <> -1) do 
         begin { draw horizontal lines }

         line(screen, r.s.x, lgridsy[yi], r.e.x, lgridsy[yi], yellow);
         yi := yi + 1 { next }
...
begin
   ...
   if button[bdots].act then begin { dot grid is active }
      ...
      yi := 1;
      { find first grid position in region }
      while (dgridsx[xi] < r.s.x) and (dgridsx[xi] <> -1) do 
         xi := xi + 1; 
      while (dgridsy[yi] < r.s.y) and (dgridsx[yi] <> -1) do 
         yi := yi + 1; 
      yis := yi; { save y }
      { lay down grid }
      while (dgridsx[xi] <= r.e.x) and (dgridsx[xi] <> -1) do 
         begin

         while (dgridsy[yi] <= r.e.y) and (dgridsy[yi] <> -1) do 
            begin

            { place grid dot }
            setpix(screen, dgridsx[xi], dgridsy[yi], black);
            yi := yi + 1 { next y }
