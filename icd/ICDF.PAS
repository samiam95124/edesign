...
procedure setchr(v: viewport; x, y: integer; ch: char; cl: color); external;
{ draw button frame }
...
begin
   ...
   viewc(vp.e, targvp);
   box(vp.s.x, vp.s.y, vp.e.x, vp.e.y, brown)
...
begin
   ...
   l := 0; { clear length }
   { find total length }
   for i := 1 to i do l := l+alphal[ord(s[i])]+1;
...
begin
   ...
   frame(((maxx-minx) div 2)-(max div 2), y, 
         ((maxx-minx) div 2)+(max div 2), y+16+(12*16)+16, 
         baklgt, baklgt, bakshw, bakshw);
   block(screen, ((maxx-minx) div 2)-(max div 2)+2, y+2, 
         ((maxx-minx) div 2)+(max div 2)-2, y+16+(12*16)+16-2, 
         bakclr);
